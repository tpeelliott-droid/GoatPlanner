  ...
  return (
    <div className="px-4 py-4">
      ...
      <div className="mb-3 flex items-start justify-between">
        <div>
          <h2 className="font-display text-xl text-parchment">{person.name}</h2>
        ...
        <button onClick={toggleStar} aria-label="Star contact">
          <Star size={20} className={person.starred ? "fill-gold text-gold" : "text-parchment/30"} />
        </button>
      </div>

      <div className="mb-5 flex gap-2">
        {person.phone && (
          <a href={`tel:${person.phone}`} className="flex items-center gap-1.5 rounded-full bg-fairway/60 px-3 py-1.5 text-xs text-parchment">
            <Phone size={13} /> Call
          </a>
        )}
        {person.phone && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center gap-1.5 rounded-full bg-fairway/60 px-3 py-1.5 text-xs text-parchment"
          >
            <MessageCircle size={13} /> WhatsApp
          </a>
        )}
        {person.email && (
          <a href={`mailto:${person.email}`} className="flex items-center gap-1.5 rounded-full bg-fairway/60 px-3 py-1.5 text-xs text-parchment">
            <Mail size={13} /> Email
          </a>
        )}
      </div>
      ...
      {person.howWeKnowThem && (
        <p className="mb-4 text-sm text-parchment/60">{person.howWeKnowThem}</p>
      )}
